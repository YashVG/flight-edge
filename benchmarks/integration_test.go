...
func TestConcurrentQueries50Workers(t *testing.T) {
	...
	// This is a saturation characterization, not the edge SLO scenario. Keep a
	// conservative throughput floor so shared CI hardware does not create a
	// false latency regression.
	assert.Greater(t, stats.QueriesPerSec, 500.0, "stress throughput should remain above 500 QPS")
}
